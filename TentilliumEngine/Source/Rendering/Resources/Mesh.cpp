...
{ 
	...
	if (tCoord_data) 
		m_vbo.tCoord = genBuffer(GL_ARRAY_BUFFER, vertex_data.size(), &(*tCoord_data)[0]);
	if (normal_data) 
		m_vbo.normal = genBuffer(GL_ARRAY_BUFFER, vertex_data.size(), &(*normal_data)[0]);
	if (colour_data) 
		m_vbo.colour = genBuffer(GL_ARRAY_BUFFER, vertex_data.size(), &(*colour_data)[0]);
...
// TODO: move function to scene.cpp
Mesh::Mesh(void* ai_mesh)
{ 
	...
	/*
	// testing
	std::vector<unsigned int> i_test_data = std::vector<unsigned int>(mesh->mNumFaces * 3);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_vbo.index);
	glGetBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, mesh->mNumFaces * 3, &i_test_data[0]);

	std::vector<float> v_test_data = std::vector<float>(mesh->mNumVertices * 3);
	glBindBuffer(GL_ARRAY_BUFFER, m_vbo.vertex);
	glGetBufferSubData(GL_ARRAY_BUFFER, 0, mesh->mNumVertices * 3, &v_test_data[0]);

	std::vector<float> t_test_data = std::vector<float>(mesh->mNumVertices * 2);
	glBindBuffer(GL_ARRAY_BUFFER, m_vbo.tCoord);
	glGetBufferSubData(GL_ARRAY_BUFFER, 0, mesh->mNumVertices * 2, &t_test_data[0]);
	*/
}

Mesh::~Mesh()
{
	// manager vbo destruction
}
